...
import {getValueInformation, isStaticMemberExpression} from "../Util";
...
feature.addPhase().after.onMemberExpression((node:AstNode<MemberExpression, any>) => {
    ...
    const propertyValue = isStaticMemberExpression(expression) ? new KnownValue(expression.property.name) : getValueInformation(expression.property);
